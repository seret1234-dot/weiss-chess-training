...
function snakeDistributeIntoChunks(items, chunkSize) {
...
async function main() {
  ...
  for await (const line of rl) {
    ...
    // For these puzzles we want at least:
    // [0] pre-move, [1] actual mate-in-1 solution
  ...
  // Snake-distribute so each chunk gets a spread of ratings
  const chunks = snakeDistributeIntoChunks(trimmed, CHUNK_SIZE)
  ...
  for (let i = 0; i < chunks.length; i++) {
    ...
    console.log(`Wrote ${filename} (${chunkData.length} puzzles)`)
  }
  ...
  const manifest = {
    ...
    distribution: 'sorted by rating, then snake-distributed across chunks',
