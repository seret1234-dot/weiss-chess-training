...
export default function KPKTrainer() {
  const trainerId = "kpk";
  const title = "KPK — King and Pawn vs King";
  const dataPath = "/data/endgames/kpk";
  const boardId = "KPKTrainerBoard";
  ...
  function shouldAskWinDraw(position: TrainingPosition | null = currentPosition) {
    // KPK uses the same generated dataset for both tasks:
  ...
  async function playEngineReplyIfNeeded(
    ...
    setMessage(
      replyInfo?.mate
        ? `Black played ${defensiveReply}. KPK result active.`
  ...
  async function showHintAction() {
    ...
    let hintMoveUci = chooseHintMove(
      game,
      currentPosition,
      currentChunkFile,
      analysis,
    ...
    // Fallback: if engine is not ready at puzzle start, give any legal move
    // that keeps the trainer responsive.
    if (!hintMoveUci) {
      const legalMoves = game.moves({ verbose: true }) as any[];
      const firstLegal = legalMoves.find((m) => m.color === game.turn());

      if (firstLegal) {
        hintMoveUci = `${firstLegal.from}${firstLegal.to}${firstLegal.promotion ?? ""}`;
      }
    }
...
function onDrop(sourceSquare: string, targetSquare: string) {
    ...
    if (nextGame.isStalemate()) {
      showWrongAndReset(
        nextGame,
        move,
        "Wrong move — try to preserve the KPK result.",
    ...
    void (async () => {
      ...
      if (!validation.ok) {
...

        {
          ...
          showWrongAndReset(nextGame, move, "Wrong move — try to preserve the KPK result.", validation.reason);
        }
        return;
      }
