...
INPUT_JSON = r"C:\Users\Ariel\chess-trainer\bn_manual_sets\bn positions_24-3-2026\bk_b8_mate6_reduced.json"

OUTPUT_JSON = r"C:\Users\Ariel\chess-trainer\bn_manual_sets\bn positions_24-3-2026\bk_b8_mate7_reduced.json"
REMOVED_JSON = r"C:\Users\Ariel\chess-trainer\bn_manual_sets\bn positions_24-3-2026\bk_b8_mate7_removed_similar.json"
...
def main():
    ...
    mate7 = [x for x in data if x.get("mate_distance") == 7]
    others = [x for x in data if x.get("mate_distance") != 7]

    print(f"Total positions in input: {len(data)}")
    print(f"Mate in 7 positions: {len(mate7)}")
    ...
    reduced_mate7 = list(kept_by_signature.values())
    reduced_mate7.sort(key=lambda x: (x["white_king"], x["white_knight"], x["white_bishop"]))

    final_output = others + reduced_mate7
    ...
    print("=== FINISHED ===")
    print(f"Original mate in 7 count: {len(mate7)}")
    print(f"Reduced mate in 7 count: {len(reduced_mate7)}")
