...
INPUT_JSON = r"C:\Users\Ariel\chess-trainer\bn_manual_sets\bn positions_24-3-2026\bk_b8_typical_w_near_king_all_mates.json"
OUTPUT_JSON = r"C:\Users\Ariel\chess-trainer\bn_manual_sets\bn positions_24-3-2026\bk_b8_mate6_reduced.json"
REMOVED_JSON = r"C:\Users\Ariel\chess-trainer\bn_manual_sets\bn positions_24-3-2026\bk_b8_mate6_removed_similar.json"
...
def choose_better(a, b):
    """
    Keep the more representative one.
    Rule:
    1. bishop closer to black king
    2. then alphabetically smaller bishop square
    """
...
def main():
    ...
    mate6 = [x for x in data if x.get("mate_distance") == 6]
    others = [x for x in data if x.get("mate_distance") != 6]

    print(f"Total positions in input: {len(data)}")
    print(f"Mate in 6 positions: {len(mate6)}")
    ...
    reduced_mate6 = list(kept_by_signature.values())
    reduced_mate6.sort(key=lambda x: (x["white_king"], x["white_knight"], x["white_bishop"]))

    final_output = others + reduced_mate6
    ...
    print("=== FINISHED ===")
    print(f"Original mate in 6 count: {len(mate6)}")
    print(f"Reduced mate in 6 count: {len(reduced_mate6)}")
