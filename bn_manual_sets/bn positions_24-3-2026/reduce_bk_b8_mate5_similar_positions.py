...
INPUT_JSON = r"C:\Users\Ariel\chess-trainer\bn_manual_sets\bn positions_24-3-2026\bk_b8_mate4_reduced.json"

OUTPUT_JSON = r"C:\Users\Ariel\chess-trainer\bn_manual_sets\bn positions_24-3-2026\bk_b8_mate5_reduced.json"
REMOVED_JSON = r"C:\Users\Ariel\chess-trainer\bn_manual_sets\bn positions_24-3-2026\bk_b8_mate5_removed_similar.json"
...
def main():
    ...
    mate5 = [x for x in data if x.get("mate_distance") == 5]
    others = [x for x in data if x.get("mate_distance") != 5]

    print(f"Total positions in input: {len(data)}")
    print(f"Mate in 5 positions: {len(mate5)}")
    ...
    reduced_mate5 = list(kept_by_signature.values())
    reduced_mate5.sort(key=lambda x: (x["white_king"], x["white_knight"], x["white_bishop"]))

    final_output = others + reduced_mate5
    ...
    print("=== FINISHED ===")
    print(f"Original mate in 5 count: {len(mate5)}")
    print(f"Reduced mate in 5 count: {len(reduced_mate5)}")
