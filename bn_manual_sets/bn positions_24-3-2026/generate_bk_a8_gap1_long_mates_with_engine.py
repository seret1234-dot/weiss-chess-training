...
OUTPUT_JSON = os.path.join(
    OUTPUT_DIR,
    "bk_a8_gap1_long_mates_with_engine.json"
)
...
def get_white_king_candidates_gap1():
    # "1 square gap" => Chebyshev distance 2 from a8
    candidates = []
    for sq in SQUARES:
        if sq == BLACK_KING:
            continue
...
def get_knight_candidates_controlling_any_near_bk_square(white_king: int):
    ...
    for knight_sq in SQUARES:
        ...
        controlled_targets = [sq for sq in near_bk if knight_attacks_square(knight_sq, sq)]
...
def main():
    ...
    white_king_candidates = get_white_king_candidates_gap1()
    near_bk = get_black_neighbor_squares()
        ...
        for white_king in white_king_candidates:
            ...
            for white_knight, controlled_targets in knight_candidates:
                ...
                for white_bishop in SQUARES:
                    ...
                    all_results.append({
                        "theme": "bn_gap1_long_mates_with_engine",
                        "fen": board.fen(),
                        ...
                        "knight_controls_near_bk_squares": [square_name(sq) for sq in controlled_targets],
                        "mate_distance": md,
    ...
    all_results.sort(key=lambda x: (x["mate_distance"], x["white_king"], x["white_knight"], x["white_bishop"]))

    with open(OUTPUT_JSON, "w", encoding="utf-8") as f:
        json.dump(all_results, f, indent=2, ensure_ascii=False)

    print("=== FINISHED ===")
    print(f"Total legal candidates checked: {total_candidates}")
