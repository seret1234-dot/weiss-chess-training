...
INPUT_JSON = r"C:\Users\Ariel\chess-trainer\bn_manual_sets\bn positions_24-3-2026\bk_b8_mate8_reduced.json"

OUTPUT_JSON = r"C:\Users\Ariel\chess-trainer\bn_manual_sets\bn positions_24-3-2026\bk_b8_mate4_reduced.json"
REMOVED_JSON = r"C:\Users\Ariel\chess-trainer\bn_manual_sets\bn positions_24-3-2026\bk_b8_mate4_removed_similar.json"
...
def main():
    ...
    mate4 = [x for x in data if x.get("mate_distance") == 4]
    others = [x for x in data if x.get("mate_distance") != 4]

    print(f"Total positions in input: {len(data)}")
    print(f"Mate in 4 positions: {len(mate4)}")
    ...
    reduced_mate4 = list(kept_by_signature.values())
    reduced_mate4.sort(key=lambda x: (x["white_king"], x["white_knight"], x["white_bishop"]))

    final_output = others + reduced_mate4
    ...
    print("=== FINISHED ===")
    print(f"Original mate in 4 count: {len(mate4)}")
    print(f"Reduced mate in 4 count: {len(reduced_mate4)}")
