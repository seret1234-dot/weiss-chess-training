...
OUTPUT_JSON = os.path.join(
    OUTPUT_DIR,
    "bk_a8_gap2_long_mates_with_engine.json"
)
...
def get_white_king_candidates_gap2():
    # "2 squares gap" => Chebyshev distance 3 from a8
    candidates = []
    for sq in SQUARES:
        if sq == BLACK_KING:
            continue
...
def analyze_mate(engine, board: Board):
    try:
        ...
        # White to move and winning mate only
        if mate_score <= 0:
            return None
        ...
        for white_king in white_king_candidates:
            ...
            for white_knight, controlled_targets in knight_candidates:
                ...
                for white_bishop in SQUARES:
                    ...
                    all_results.append({
                        "theme": "bn_gap2_long_mates_with_engine",
                        "fen": board.fen(),
