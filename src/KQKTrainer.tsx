...
const PROGRESS_KEY = 'kqk_trainer_progress_v4'
const BOARD_WIDTH_KEY = 'kqk_trainer_board_width_v4'
...
function buildChunks(): TrainerChunk[] {
  const file = kqkData as RawFile

  return [...file.groups]
    .sort((a, b) => a.mateIn - b.mateIn)
    .map((group) => ({
      id: `kqk_m${group.mateIn}`,
      label: `Mate in ${group.mateIn}`,
      mateDistance: group.mateIn,
      puzzles: group.positions.map((p, index) => ({
        id: `kqk_m${group.mateIn}_${index + 1}`,
...
async function saveProgress(progressMap: Record<string, PuzzleProgress>) {
  ...
  if (error) {
    console.error('Failed to save KQK progress:', error)
  }
}

export default function KQKTrainer() {
  ...
  function onDrop(sourceSquare: string, targetSquare: string) {
    ...
    void (async () => {
      ...
      const baselineMate = Math.max(
        currentPuzzle.mateDistance,
        engineStartMate ?? currentPuzzle.mateDistance
      )
      ...
      if (!accepted) {
        ...
        await showWrongAndReset(
          ...
          'Wrong move.',
          startBestUci
            ? `Hint move was ${startBestUci}. Good moves must keep the mate or preserve Black's box.`
  ...
  return (
    <TrainerShell
      title="K + Q vs K Trainer"
      subtitle={currentChunk?.label ?? 'K + Q vs K'}
      ...
      footerLeft={currentChunk?.label ?? 'K + Q vs K'}
      footerRight={`${boardWidth}px`}
      boardId="KQKTrainerBoard"
      fen={game.fen()}
      onPieceDrop={onDrop}
        ...
        >
          ...
          <PanelCard>
            ...
            <div style={{ fontSize: 12, color: '#d0d0d0', lineHeight: 1.55 }}>
              ...
              <div>Chunk target: mate in {currentChunk?.mateDistance ?? '-'}</div>
              <div>Puzzle target: mate in {currentPuzzle?.mateDistance ?? '-'}</div>
