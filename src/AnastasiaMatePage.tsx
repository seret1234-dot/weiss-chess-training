...
const DATA_BASE_PATH = '/data/lichess/mate_in_1/anastasia_fixed'
const MANIFEST_FETCH_PATH = `${DATA_BASE_PATH}/manifest.json`
const STORAGE_KEY = 'mate_trainer_anastasia_progress_v1'
...
function normalizePuzzle(raw: LichessChunkPuzzle, index: number): DemoPuzzle | null {
  ...
  const finalFullSolution =
    solutionFromArray.length > 0
      ? solutionFromArray
      : rawMoves.length > 0
        ? rawMoves
        : [solutionMove]
  ...
  return {
    ...
    preMove,
    solutionMove,
    fullSolution: finalFullSolution,
    chunkNumber,
    chunkIndex,
    userMoveCount: countUserMovesFromFullSolution(finalFullSolution),
  }
}
...
export default function AnastasiaMatePage() {
  ...
  function jumpToChunk() {
    ...
    const targetIndex = Math.max(0, Math.min(chunkFiles.length - 1, Math.floor(parsed) - 1))
  ...
  const mainProgressWidth = `${currentChunkTarget > 0 ? (totalFastSolves / currentChunkTarget) * 100 : 0}%`
              ...
              <div style={{ display: 'flex', gap: 8 }}>
                <button
                  ...
                  style={{
                    ...
                    padding: '12px 14px',
                    background:
                      loading || currentChunkIndex <= 0 ? '#2e2a28' : '#3b3734',
                    color:
                      loading || currentChunkIndex <= 0 ? '#8f8a86' : 'white',
                    fontSize: 14,
                    fontWeight: 700,
                    cursor:
                      loading || currentChunkIndex <= 0 ? 'default' : 'pointer',
                  }}
                >
                  Previous
                </button>
                ...
                <button
                  ...
                  style={{
                    ...
                    padding: '12px 14px',
                    background:
                      loading || currentChunkIndex >= chunkFiles.length - 1
                        ? '#2e2a28'
                        : '#3b3734',
                    color:
                      loading || currentChunkIndex >= chunkFiles.length - 1
              ...
              <div style={{ display: 'flex', gap: 8 }}>
                ...
                <button
                  ...
                  style={{
                    ...
                    borderRadius: 10,
                    padding: '12px 16px',
                    background: loading || chunkFiles.length === 0 ? '#2e2a28' : '#7a5f2b',
                    color: loading || chunkFiles.length === 0 ? '#8f8a86' : 'white',
                    fontSize: 14,
                    fontWeight: 700,
                    cursor:
                      loading || chunkFiles.length === 0 ? 'default' : 'pointer',
                  }}
                >
                  Jump
                </button>
