...
type DetectionResult = {
 ...
 sideToMove?: "w" | "b";
 castling?: string;
 orientation?: "white_bottom" | "black_bottom" | "unknown";
 confidence?: number;
 notes?: string;
 model?: string;
};
...
export default function ImageToPositionPanel({
 ...
 const [autoDetectOnUpload, setAutoDetectOnUpload] = useState(true);
 ...
 "Upload a board image. The app will try to detect the FEN automatically, then you can correct it if needed.",
 );
 ...
 function applyFenToEditor(nextFen: string) {
 const parsed = new Chess(nextFen);
 const normalizedFen = parsed.fen();
 setBoard(fenToBoardMap(normalizedFen));
 setSideToMove(normalizedFen.split(" ")[1] === "b" ? "b" : "w");
 setCastling(normalizeCastling(normalizedFen.split(" ")[2] || "-"));
 setDetectedFen(normalizedFen);
 }

 async function detectPositionFromDataUrl(dataUrl: string) {
 if (!dataUrl) {
 ...
 setIsDetecting(true);
 setStatus("Detecting board and pieces automatically...");

 try {
 const { data, error } = await supabase.functions.invoke<DetectionResult>("image-to-fen", {
 body: { imageDataUrl: dataUrl },
 });

 if (error) throw new Error(error.message);
 if (!data?.fen) throw new Error("No FEN returned from image detector");

 applyFenToEditor(data.fen);

 if (data.orientation === "black_bottom") setOrientation("black");
 if (data.orientation === "white_bottom") setOrientation("white");

 const confidence = typeof data.confidence === "number"
 ? ` Confidence: ${Math.round(data.confidence * 100)}%.`
 : "";

 setStatus(`Position detected automatically.${confidence} Check side to move/castling, correct pieces if needed, then load into Analyze.`);
 } catch (err) {
 setDetectedFen("");
 setStatus(`Automatic detection failed: ${err instanceof Error ? err.message : "unknown error"}. You can still correct the board manually.`);
 } finally {
 ...
 await detectPositionFromDataUrl(dataUrl);
 } else {
 setStatus("Image loaded. Click Detect Automatically, or use the piece palette manually.");
 }
 } catch {
 ...
 Automatic detection first, with manual correction if needed.
 </div>
 </div>
 ...
 >
 Upload a board image. The app will detect the position automatically, then you can correct if needed.
 </div>
 )}
 ...
 <input
 type="checkbox"
 checked={autoDetectOnUpload}
 onChange={(e) => setAutoDetectOnUpload(e.target.checked)}
 />
 Detect automatically after upload
 </label>

 <div style={{ marginTop: 8 }}>
 <PrimaryButton
 onClick={() => detectPositionFromDataUrl(lastImageDataUrl)}
 disabled={!lastImageDataUrl || isDetecting}
 >
