...
export default function LucenaTrainer() {
  const trainerId = "lucena";
  const title = "Lucena";
  const dataPath = "/data/endgames/lucena";
  const boardId = "LucenaTrainerBoard";
  ...
  function shouldAskWinDraw(position: TrainingPosition | null = currentPosition) {
    // KRKP uses the same generated dataset for both tasks:
  ...
  function whiteRookStillExists(gameToCheck: Chess) {
    return gameToCheck
      .board()
      .some((rank) =>
        rank.some((piece) => piece?.type === "r" && piece.color === "w"),
      );
  }
  ...
  function blackCanCaptureWhiteRook(gameToCheck: Chess) {
    ...
    return moves.some((m) => m.color === "b" && m.captured === "r");
  }

  function userMoveCompletesGoal(nextGame: Chess) {
    if (nextGame.isCheckmate()) return true;

    const pawnGone = !blackPawnStillExists(nextGame);
    const rookAlive = whiteRookStillExists(nextGame);

    if (pawnGone && rookAlive && !blackCanCaptureWhiteRook(nextGame)) {
      return true;
    }
  ...
  async function playEngineReplyIfNeeded(
    ...
    if (!whiteRookStillExists(replyResult.game) && blackPawnStillExists(replyResult.game)) {
      showWrongAndReset(
        replyResult.game,
        replyResult.move,
        "Wrong move.",
        "Black won your rook. This position is now lost."
      );
      return;
    }
  ...
  async function showHintAction() {
    ...
    setMessage(
      `Try ${parsed.from} ? ${parsed.to}${
        parsed.promotion ? ` (${parsed.promotion})` : ""
      }`,
    );
  }

  async async function validateByEngine(
    beforeFen: string,
    afterFen: string,
    attemptedUci: string,
    nextGame: Chess,
  ...
  useRegisterPlayableBoard({useRegisterPlayableBoard({
    fen: game.fen(),
