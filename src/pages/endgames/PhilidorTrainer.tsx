...
function getInstructionText(
 ...
 return "Hold the draw for 20 moves";
}
...
export default function PhilidorTrainer() {
 const trainerId = "philidor";
 const title = "Philidor";
 const dataPath = "/data/endgames/philidor";
 const boardId = "PhilidorTrainerBoard";
...
const REQUIRED_DEFENSE_MOVES = 20;
 ...
 function moveChangesPawnOrCaptures(baseFen: string, uci: string) {
 const parsed = parseUciMove(uci);
 if (!parsed) return true;

 const test = new Chess(baseFen);
 const moveObj = test.move({
 from: parsed.from,
 to: parsed.to,
 promotion: parsed.promotion,
 });
 ...
 if (shouldAskWinDraw(currentPosition)) {
 ...
 "Goal: Hold the draw as Black for 20 defensive moves. Pawn moves are not allowed. Captures are allowed.",
 );
 setInputLocked(false);
 }
 ...
 setMessage("Try again. The goal is to hold the draw.");
 setLastMove({});
 clearHighlights();
 setInputLocked(false);
 setMoveTimesMs([]);
 ...
 ? "Promoted!"
 : PhilidorBridgeAchieved(nextGame)
 ? "Bridge achieved!"
 : "Correct."
 );
 setMessage(
 ...
 function PhilidorBridgeAchieved(gameToCheck: Chess) {
 ...
 function userMoveCompletesGoal(nextGame: Chess, engineResult?: EngineResult | null) {
 if (nextGame.isDraw() || nextGame.isStalemate()) return true;

 if (
 engineResult?.mate !== null &&
 engineResult?.mate !== undefined
 ) {
 return false;
 }
 ...
 Math.abs(engineResult.eval) > 1.5
 ) {
 return false;
 }

 return defenseMoves >= REQUIRED_DEFENSE_MOVES;
}
 ...
 if (userMoveCompletesGoal(afterUserGame, afterUserInfo ?? null)) {
 handleSolved(afterUserGame, userMove);
 return;
 }

 const replyInfo =
 afterUserInfo ?? (await evaluatePosition(afterUserGame.fen()));
 ...
 setLastMove({ from: userMove.from, to: userMove.to });
 clearHighlights();
 setStatus(`Good move. Defense: ${Math.min(defenseMoves + 1, REQUIRED_DEFENSE_MOVES)}/${REQUIRED_DEFENSE_MOVES}`);
 ...
 setMessage("Wait for the engine reply. You only play Black.");
 return;
 }

 const legalNow = getLegalUciMoves(game).filter(
 (uci) => !moveChangesPawnOrCaptures(game.fen(), uci),
 );
 ...
 ...legalNow,
 ].filter((uci): uci is string => !!uci && legalNow.includes(uci));

 let hintMoveUci: string | null = null;

 for (const uci of Array.from(new Set(candidates))) {
 ...
 if (!hintMoveUci) {
 setStatus("Hint");
 setMessage("No accepted hint move found.");
 return;
 }
 ...
 setStatus("Hint");
 setMessage(
 `Accepted defensive move: ${parsed.from} ? ${parsed.to}${parsed.promotion ? ` (${parsed.promotion})` : ""}`,
 );
}
...
async function validateByEngine(
 ...
 if (moveChangesPawnOrCaptures(beforeFen, attemptedUci)) {
 return {
 ok: false,
 reason: "Pawn moves are not allowed. Captures are allowed in this Philidor drill.",
 afterUser: null,
 };
 }
 ...
 return { ok: true, reason: "Move keeps the draw.", afterUser };
 }

 return {
 ok: false,
 reason: before.bestMove
 ? `Use the engine-best Philidor move: ${before.bestMove}`
 : "This move loses the draw.",
 afterUser,
 };
}
...
if (!currentPosition || !progress || !currentThemeConfig) return false;
 if (game.turn() !== "b") {
 setStatus("White to move.");
 setMessage("White engine reply is still pending. You only play Black.");
 return false;
 }
 if (
 loadingChunk ||
 ...
 if (
 (strictChunk || hasPreparedRoute) &&
 legalAllowedMoves.includes(attemptedUci)
 ) {
 ...
 console.error("Philidor black reply crashed:", err);
 console.error("Philidor black reply crashed:", err);
 ...
 arePiecesDraggable={
 !loadingChunk && !inputLocked && !allComplete && phase === "play"
 }
