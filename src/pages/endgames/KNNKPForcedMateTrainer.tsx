...
export default function KNNKPForcedMateTrainer() {
 const trainerId = "knnkp_forced_mate";
 const title = "KNN vs KP - Forced Mate";
 const dataPath = "/data/endgames/knnkp/forced-mate";
 const boardId = "KNNKPForcedMateTrainerBoard";
 ...
 console.error("KNNKP engine analysis failed:", err);
 return null;
 }
 }
 ...
 // KNNKP uses the same generated dataset for both tasks:
 // draw positions are evaluation questions, win positions are playable conversion drills.
 if (position?.result === "draw") return true;
 return false;
 }
 ...
 // KNNKP forced mate: only checkmate completes the drill.
 ...
 setMessage(
 replyInfo?.mate
 ? `Black played ${defensiveReply}. Mate countdown active.`
 : `Black played ${defensiveReply}. Your move.`
 );
 setInputLocked(false);
 setJustMated(false);
 setMateCountdown(null);
 ...
 setStatus("Hint");
 setMessage("Looking for best move...");
 ...
 // Fast fallback for mate-in-1 before engine is ready.
 for (const m of legalMoves) {
 const testGame = new Chess(game.fen());
 const made = testGame.move({
 from: m.from,
 to: m.to,
 promotion: m.promotion,
 });

 if (made && testGame.isCheckmate()) {
 setMarkedSquare(m.from);
 setHintSquares([m.to]);
 setEscapeSquares([]);
 setMessage(
 `Mate in 1: ${m.from} ? ${m.to}${m.promotion ? ` (${m.promotion})` : ""}`,
 );
 return;
 }
 }
 ...
 if (!hintMoveUci) {
 setMessage("No legal hint available yet. Try again in a second.");
 return;
 }
 ...
 setEscapeSquares([]);
 setMessage(
 `Best move: ${parsed.from} ? ${parsed.to}${
...
if (!currentPosition || !progress || !currentThemeConfig) return false;
 ...
 showWrongAndReset(nextGame, move, "Wrong move - try to reduce mate distance.", validation.reason);
 }
 return;
 }
