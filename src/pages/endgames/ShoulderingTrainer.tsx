...
function getInstructionText(
 ...
 if (position.mateDistance === 1) return "Find the winning shouldering move";
 return `Find the winning shouldering move`;
 }
 ...
 return position.result === "draw" ? "Hold the draw" : "Find the winning shouldering move";
}
...
export default function ShoulderingTrainer() {
 const trainerId = "shouldering";
 const title = "KPK - Shouldering";
 const dataPath = "/data/endgames/shouldering";
 const boardId = "ShoulderingTrainerBoard";
 ...
 : "Position restarted. Win the shouldering.",
 );
 }
 ...
 setInputLocked(false);
 setStatus(getInstructionText(currentPosition, currentThemeConfig));
 setMessage(currentPosition.explanation || "Correct - now Win the shouldering.");
 setCurrentMoveElapsedMs(0);
 setMoveTimesMs([]);
 moveStartedAtRef.current = Date.now();
 void analyzeCurrentFen(currentPosition.startFen);
...
function countWhitePawns(gameToCheck: Chess) {
 return gameToCheck
 .board()
 .reduce(
 (total, row) =>
 total + row.filter((piece) => piece?.type === "p" && piece.color === "w").length,
 0,
 );
}
...
function userMoveCompletesGoal(nextGame: Chess) {
 ...
 if (countWhitePawns(replyResult.game) < countWhitePawns(afterUserGame)) {
 showWrongAndReset(
 replyResult.game,
 userMove,
 "Wrong - pawn lost.",
 "Black captured the pawn. In this drill the pawn must survive.",
 );
 return;
}
setGame(replyResult.game);
 ...
 return {
 ok: false,
 reason: "This move does not keep the shouldering advantage.",
 afterUser,
 };
 }
