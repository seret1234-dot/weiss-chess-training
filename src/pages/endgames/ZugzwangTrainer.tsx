...
function getInstructionText(
 ...
 if (goal === "checkmate") return "Convert the KPK result";

 if (typeof position.mateDistance === "number") {
 if (position.mateDistance === 1) return "Find the winning zugzwang move";
 return `Find the winning zugzwang move`;
 }
 ...
 return position.result === "draw" ? "Hold the draw" : "Find the winning zugzwang move";
}
...
export default function ZugzwangTrainer() {
 const trainerId = "zugzwang";
 const title = "KPK -  Zugzwang";
 const dataPath = "/data/endgames/zugzwang";
 const boardId = "ZugzwangTrainerBoard";
 const safeDataPath = dataPath.replace(/\/+$/, "");
 const boardWidthKey = `${trainerId}_boardWidth_v3`;
 const progressKey = `${trainerId}_progress_v4`;
 ...
 "Use stalemate, key squares, king activity, and correct pawn timing.",
 );
 setInputLocked(false);
 }
 ...
 : "Position restarted. Win the pawn race.",
 );
 }
 ...
 setInputLocked(false);
 setStatus(getInstructionText(currentPosition, currentThemeConfig));
 setMessage(currentPosition.explanation || "Correct -  now Win the pawn race.");
 setCurrentMoveElapsedMs(0);
 setMoveTimesMs([]);
 moveStartedAtRef.current = Date.now();
 void analyzeCurrentFen(currentPosition.startFen);
 ...
 // Prefer king moves in KPK because the defender is only a king.
 ...
 setStatus("Hint");
 setMessage("Checking legal winning moves...");
 setInputLocked(true);
 ...
 try {
 analysis = await evaluatePosition(beforeFen);
 ...
 candidates.push(analysis.bestMove);
 }
 ...
 1800,
 { ok: false, reason: "Engine timeout", afterUser: null },
 ) as any;

 if (!validation.ok) continue;

 const movedPiece =
 game.get(parsed.from as any)?.type ?? "";

 const fromRank = Number(parsed.from[1]);
 const toRank = Number(parsed.to[1]);
 const isWhitePawnPush =
 movedPiece === "p" && parsed.from[0] === parsed.to[0] && toRank > fromRank;

 const isPromotion = movedPiece === "p" && toRank === 8;

 const scoreBase =
 typeof validation.afterUser?.mate === "number"
 ? 100000 - Math.abs(validation.afterUser.mate)
 : typeof validation.afterUser?.eval === "number"
 ? validation.afterUser.eval
 : attemptedUci === analysis?.bestMove
 ? 50000
 : 0;
 ...
 scoreBase +
 (isPromotion ? 20000 : 0) +
 (isWhitePawnPush ? 12000 : 0) +
 (movedPiece === "k" ? -3000 : 0);

 if (score > bestScore) {
 bestScore = score;
 bestHint = attemptedUci;
 }
 }

 if (!bestHint) {
 setStatus("Hint unavailable");
 setMessage("No validated safe hint found in this position.");
 setInputLocked(false);
 return;
 }

 lastHintMoveRef.current = bestHint;

 const parsed = parseUciMove(bestHint);
 if (!parsed) {
 setStatus("Hint");
 setMessage(`Suggested move: ${bestHint}`);
 setInputLocked(false);
 return;
 }
 ...
 setStatus("Hint");
 setMessage(
 `Safe move: ${parsed.from} ? ${parsed.to}${parsed.promotion ? ` (${parsed.promotion})` : ""}`,
 );
 setInputLocked(false);
 }
 ...
 function blackCanCaptureWhitePawn(gameToCheck: Chess) {
 const whitePawnSquare = getPieceSquare(gameToCheck, "p", "w");
 if (!whitePawnSquare) return !hasWhiteQueenExact(gameToCheck);

 const moves = gameToCheck.moves({ verbose: true }) as any[];
 return moves.some(
 (m) => m.color === "b" && m.to === whitePawnSquare && m.captured === "p",
 );
 }
 ...
 if (!whitePawnStillExists(nextGame)) {
 return { ok: false, reason: "This move loses the pawn.", afterUser: null };
 }

 if (blackCanCaptureWhitePawn(nextGame)) {
 return { ok: false, reason: "This move lets Black take the pawn.", afterUser: null };
 }
 }
 ...
 const winningByMate =
 typeof afterUser.mate === "number" && Math.abs(afterUser.mate) > 0;

 const winningByEval =
 typeof afterUser.eval === "number" && Math.abs(afterUser.eval) >= 1.2;

 if (winningByMate || winningByEval) {
 return { ok: true, reason: "", afterUser };
 }

 return {
 ok: false,
 reason: "This move does not keep the zugzwang advantage.",
 afterUser,
 };
 }
...
if (!currentPosition) return false;
 ...
 // In KPK trainer the user plays White. Black must auto-reply.
 if (!engineInfo) {
 setStatus("Engine is still checking...");
 setMessage("Wait a moment, then play.");
 return false;
 }
 ...
 validateByEngine(beforeFen, nextGame.fen(), attemptedUci, nextGame),
 900,
 { ok: false, reason: "Engine is still checking. Try again.", afterUser: null },
 );
