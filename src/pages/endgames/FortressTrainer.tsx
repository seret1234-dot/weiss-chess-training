...
function getInstructionText(
 ...
 if (goal === "checkmate") return "Convert the Fortress result";

 if (typeof position.mateDistance === "number") {
 if (position.mateDistance === 1) return "Find the mate in 1";
 return `Find the winning pawn race move (mate in ${position.mateDistance})`;
 }
 ...
 return "Find the drawing fortress move";
}
...
function isFortressStalemate(gameToCheck: Chess) {
...
export default function FortressTrainer() {
 const trainerId = "fortress";
 const title = "Fortress -  Fortress";
 const dataPath = "/data/endgames/fortress";
 const boardId = "FortressTrainerBoard";
 ...
 : "Position restarted. Find the drawing fortress move.",
 );
 }
 ...
 // Prefer king moves in Fortress because the defender is only a king.
 ...
 if (isFortressStalemate(afterUserGame)) {
 clearPendingFeedbackTimeout();
 ...
 function getFortressTargetResult(info) {
 ...
 function fastFortressOutcome(gameToCheck) {
 ...
 function isSafeFortressPawnPush(beforeGame: Chess, nextGame: Chess, move: any) {
 ...
 if (target === "win") {
 if (isSafeFortressPawnPush(beforeGame, nextGame, move)) {
 return { ok: true, reason: "", afterUser: afterInfo };
 }
...
if (!currentPosition) return false;
 ...
 // In Fortress trainer the user plays White. Black must auto-reply.
...
return (
 <TrainerShell
 FortressBoardOverlay={FortressBoardOverlay}
 title={title}
 subtitle={
