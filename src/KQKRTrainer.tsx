...
const PROGRESS_KEY = 'kqkr_trainer_progress_v1'
const BOARD_WIDTH_KEY = 'kqkr_trainer_board_width_v1'
...
const ACCEPT_MATE_SLACK = 2
const ACCEPT_BOX_SCORE_SLACK = 2
...
function buildChunks(selectedGroup?: string): TrainerChunk[] {
  const file = kqkrData as RawFile

  const filteredGroups =
    selectedGroup && selectedGroup !== "all"
      ? file.groups.filter((g) => g.key === selectedGroup)
      : file.groups
  ...
  return filteredGroups.map((group) => ({
    ...
    label: group.label,
    forkDistance: 0,
    puzzles: group.positions.map((p, index) => ({
      id: `${group.key}_${index + 1}`,
...
function getWhiteQueenSquare(game: Chess) {
  const board = game.board()
  for (let rank = 0; rank < 8; rank += 1) {
    for (let file = 0; file < 8; file += 1) {
      const piece = board[rank][file]
...
function hasBlackRook(game: Chess) {
  const board = game.board()
  for (let rank = 0; rank < 8; rank += 1) {
    for (let file = 0; file < 8; file += 1) {
      const piece = board[rank][file]
...
function getEdgeDistance(square: string) {
  const c = squareToCoords(square)
  if (!c) return 99
  return Math.min(c.file, 7 - c.file, c.rank, 7 - c.rank)
}

function getKingDistance(a: string, b: string) {
...
function forceTurnFen(fen: string, turn: 'w' | 'b') {
...
function getContainmentSnapshot(game: Chess): ContainmentSnapshot {
  const bk = getBlackKingSquare(game)
  const wk = getWhiteKingSquare(game)
  const mobility = getLegalBlackKingMoves(game).length
  const edgeDistance = getEdgeDistance(bk)
  const kingDistance = getKingDistance(wk, bk)

  const score = mobility * 10 + edgeDistance * 4 + kingDistance
...
async function saveProgress(progressMap: Record<string, PuzzleProgress>) {
  ...
  if (error) {
    console.error('Failed to save KQKR progress:', error)
  }
}

export default function KQKRTrainer({ group = "all" }: { group?: string }) {
  const chunks = useMemo(() => buildChunks(group), [group])
  ...
  const [successOverlayText, setSuccessOverlayText] = useState('ROOK WON')
  ...
  async function loadPuzzle(puzzle: TrainerPuzzle, nextOrder: number[], nextQueueIndex: number, forkDistance: number) {
    ...
    setLocked(false)
    setLastMove({})
    setStatus(`Win the rook (${currentChunk?.label ?? 'study'})`)
    setMessage('')
  ...
  async function loadChunk(
    ...
    if (nextOrder.length === 0) {
      if (options?.allowCompletedChunk) {
        ...
        await loadPuzzle(reviewPuzzle, reviewOrder, 0, chunk.forkDistance)
    ...
    await loadPuzzle(puzzle, nextOrder, nextQueueIndex, chunk.forkDistance)
  }

  useEffect(() => {
    if (chunks.length === 0 || !progressLoaded) return

    const firstIncomplete = chunks.findIndex(chunk =>
      chunk.puzzles.some(p => !isMastered(p.id))
    )
    ...
    void loadChunk(firstIncomplete)
  ...
  async function finishSuccess(successText: string, finalMoveElapsedMs?: number) {
    ...
    setLocked(true)
    setSuccessOverlayText(successText)
    setJustSolved(true)
    ...
    if (nextOrder.length > 1 && currentPuzzle) {
      ...
      await loadPuzzle(nextPuzzle, rotated, 0, currentChunk.forkDistance)
      return
    }
  ...
  async function resetPuzzle() {
    ...
    setLocked(false)
    setLastMove({})
    setStatus(`Win the rook (${currentChunk.label})`)
    setMessage('')
    setMoveTimesMs([])
    beginMoveTimer()
    await setEngineForCurrentPosition(puzzle.fen)
  ...
  async function getBestBlackReply(afterWhiteGame: Chess) {
    ...
    let blackMove = null as ReturnType<Chess['move']> | null
  ...
  function onDrop(sourceSquare: string, targetSquare: string) {
    ...
    void (async () => {
      if (!hasBlackRook(nextGame)) {
        await finishSuccess('ROOK WON!', elapsed)
        return
      }
      ...
      const baselineMate = engineStartMate ?? 10

      const beforeBlackMobilityGame = new Chess(forceTurnFen(beforeFen, 'b'))
      const beforeContainment = getContainmentSnapshot(beforeBlackMobilityGame)
      ...
      const { info: afterWhiteInfo, replyGame, blackMove, afterBlackEval } = await getBestBlackReply(nextGame)
      ...
      const exactHintMoveAccepted = !!startBestUci && attemptedUci === startBestUci

      const stillWinningForWhite =
        afterWhiteMateDistance !== null &&
        afterBlackMateDistance !== null

      const afterWhiteContainment = getContainmentSnapshot(nextGame)

      const mateAccepted =
        stillWinningForWhite &&
        afterBlackMateDistance <= baselineMate + ACCEPT_MATE_SLACK &&
        afterWhiteMateDistance <= baselineMate + ACCEPT_MATE_SLACK + 1

      const boxPreservedOrImproved =
        afterWhiteContainment.score <= beforeContainment.score + ACCEPT_BOX_SCORE_SLACK &&
        afterWhiteContainment.mobility <= beforeContainment.mobility + 1 &&
        afterWhiteContainment.kingDistance <= beforeContainment.kingDistance + 1

      const boxAccepted =
        stillWinningForWhite &&
        boxPreservedOrImproved &&
        afterBlackMateDistance <= baselineMate + ACCEPT_MATE_SLACK + 2

      const evalBefore = beforeInfo?.eval ?? 0
      const evalAfter = afterWhiteInfo?.eval ?? evalBefore

      const evalImproved = evalAfter >= evalBefore - 0.3
      const evalCloseToBest = Math.abs(evalAfter - evalBefore) <= 0.5

      let accepted = false

      // Strict calculation mode for fork-2 and fork-3:
      // only the engine best move is accepted, so these sections train exact forcing play.
      if (group === 'fork-1' || group === 'fork-2' || group === 'fork-3') {
        accepted = exactHintMoveAccepted
      } else {
        accepted =
          exactHintMoveAccepted ||
          evalImproved ||
          evalCloseToBest ||
          mateAccepted ||
          boxAccepted
      ...
      if (!accepted) {
        ...
        await showWrongAndReset(
          ...
          'Wrong move.',
          startBestUci
            ? `Hint move was ${startBestUci}. Good moves must keep the win/mate path or preserve Black's box.`
            : 'That move does not keep the winning path correctly.'
        )
        return
      }

      if (!replyGame || !blackMove) {
        // Extremely rare fallback — do not mark a good move wrong if the engine reply fails.
        setLocked(false)
        setStatus('Good move.')
        setMessage('Engine fallback applied. Continue.')
        beginMoveTimer()
        return
      }
      ...
      setStatus(
        afterBlackMateDistance !== null
          ? `Continue — mate in ${afterBlackMateDistance}`
          : 'Continue'
      )
  ...
  const queenSquare = getWhiteQueenSquare(game)
  ...
  return (
    <TrainerShell
      title="K + Q vs K + R Trainer"
      subtitle={currentChunk?.label ?? 'K + Q vs K + R'}
      ...
      footerLeft={currentChunk?.label ?? 'K + Q vs K + R'}
      footerRight={`${boardWidth}px`}
      boardId="KQKRTrainerBoard"
      fen={game.fen()}
      onPieceDrop={onDrop}
      ...
      sidePanel={
        ...
        >
          ...
          <BigMessage
            ...
            message={status || `Win the rook (${currentChunk?.label ?? 'study'})`}
          />
          ...
          <PanelCard>
            ...
            <div style={{ fontSize: 12, color: '#d0d0d0', lineHeight: 1.55 }}>
              ...
              <div>Mode: Queen vs rook technique trainer</div>
              <div>Puzzle solves: {currentProgress?.fastSolves ?? 0} / 5</div>
              <div>Total solves: {currentProgress?.totalSolves ?? 0}</div>
              <div>Section target: {currentChunk?.label ?? '-'}</div>
              <div>Puzzle target: win the rook</div>
