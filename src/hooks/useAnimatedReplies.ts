...
export function useAnimatedReplies() {
 ...
 // King moves cannot use ghostless animation because chess.js rejects FENs with a missing king.
 // Keep the real board unchanged during the animation, then swap to finalGame at the end.
 if (pieceBeforeReply.type === 'k') {
 ...
 const kingAnimationGame = new Chess(baseGame.fen())
 const pieceOnTargetBefore = baseGame.get(reply.to as Square)
 if (pieceOnTargetBefore) {
 kingAnimationGame.remove(reply.to as Square)
 }

 onPosition(kingAnimationGame)
