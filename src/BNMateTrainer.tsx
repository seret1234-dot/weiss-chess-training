...
const RESET_AT_KEY = 'bnMate_progress_reset_at_v5'
...
async function loadStoredProgress(): Promise<TrainerProgress | null> {
 ...
 const resetAtRaw = localStorage.getItem(RESET_AT_KEY)
 const resetAtMs = resetAtRaw ? Date.parse(resetAtRaw) : 0
...
if (rows.length === 0) {
 ...
 if (error) {
 console.error('Failed to clear empty BN progress:', error)
 }
...
export default function BNMateTrainer() {
 ...
 async function resetWholeProgression() {
 ...
 <button
 onClick={() => void resetWholeProgression()}
 style={{
 background: '#7a3d3d',
 color: '#fff',
 border: 'none',
 ...
 <SecondaryButton onClick={() => void resetWholeProgression()}>
