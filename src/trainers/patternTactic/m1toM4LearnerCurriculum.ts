...
export type PatternTacticLearnerCurriculum = {
  ...
  version: "m1-v1" | "m2-v1" | "m3-v1" | "m4-v1"
...
function definition(theme: string, tacticDistance: 1 | 2 | 3 | 4): PatternTacticLearnerCurriculum {
  ...
  const version = `m${tacticDistance}-v1` as PatternTacticLearnerCurriculum["version"]
  return {
    ...
    learnerDataBasePath: `/data/learner-curricula/pattern-tactics/${theme}-m${tacticDistance}-v1`,
    legacyChunkCount: LEGACY_CHUNK_COUNTS[sourceKey] ?? 10,
    activeChunkCount: ACTIVE_CHUNK_COUNT_OVERRIDES[sourceKey] ?? (tacticDistance === 1 ? 5 : 8),
