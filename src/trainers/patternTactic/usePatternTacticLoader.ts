...
import type {
 LoaderState,
 NormalizedPatternTacticPuzzle,
 PatternTacticChunkFile,
 PatternTacticManifest,
 PatternTacticPuzzle,
} from "./types"
...
function extractRawPuzzles(chunkData: unknown): PatternTacticPuzzle[] {
 if (Array.isArray(chunkData)) {
 return chunkData as PatternTacticPuzzle[]
 }

 if (
 chunkData &&
 typeof chunkData === "object" &&
 Array.isArray((chunkData as PatternTacticChunkFile).puzzles)
 ) {
 return (chunkData as PatternTacticChunkFile).puzzles
 }

 return []
}

export function usePatternTacticLoader(
 manifestPath: string,
 ...
 const manifest = (await response.json()) as PatternTacticManifest
 ...
 const map = new Map<string, NormalizedPatternTacticPuzzle>()
