...
export function normalizePuzzle(
 ...
 id: raw.id,
 fen: raw.fen,
 preMove: isNonEmptyString(raw.preMove) ? raw.preMove : undefined,
 rating: typeof raw.rating === "number" ? raw.rating : 500,
 label,
 theme: isNonEmptyString(raw.theme) ? raw.theme : fallbackTheme,
 chunkNumber: Number.isInteger(raw.chunkNumber)
 ? raw.chunkNumber
 : Number.isInteger((raw as any).chunk)
 ? (raw as any).chunk
 : 1,
 chunkIndex: Number.isInteger(raw.chunkIndex)
 ? raw.chunkIndex
 : Number.isInteger((raw as any).orderInChunk)
 ? (raw as any).orderInChunk - 1
 : 0,
 solutionLine,
 userMoveIndexes: Array.isArray(raw.userMoveIndexes) ? raw.userMoveIndexes : [0],
 }
}

export function normalizePuzzleList(
 rawPuzzles: PatternTacticPuzzle[],
 fallbackTheme = "Pattern Mate",
): NormalizedPatternTacticPuzzle[] {
