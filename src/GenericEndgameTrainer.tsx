...
export default function GenericEndgameTrainer({
 trainerId,
 title,
 dataPath,
 boardId,
}: GenericEndgameTrainerProps) {
 const safeDataPath = dataPath.replace(/\/+$/, "");
 const boardWidthKey = `${trainerId}_boardWidth_v1`;
 const progressKey = `${trainerId}_progress_v1`;
 ...
 const next =
 globalChunkLookup[Math.min(globalChunkLookup.length - 1, currentIdx + 1)];
 if (next) {
 ...
 if (whiteQueenAttacksBlackPawn(next) && !blackKingProtectsPawn(next))
 score -= 1500;
 ...
 setStatus("Hint");
 setMessage(
 `Try ${parsed.from} → ${parsed.to}${parsed.promotion ? ` (${parsed.promotion})` : ""}`,
 );
 }
