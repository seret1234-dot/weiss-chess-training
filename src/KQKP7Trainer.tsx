...
const BOARD_WIDTH_KEY = 'kqkp7_boardWidth_v1'
const PROGRESS_KEY = 'kqkp7_progress_v1'
...
function normalizePosition(raw: RawTrainingPosition, index: number, chunkFile: string): TrainingPosition {
 ...
 const label =
 raw.label ||
 `${raw.phase || raw.theme || 'kqkp7'} #${index + 1}`
...
function queenAttacksSquare(queenSquare: string, targetSquare: string, blockers = new Set<string>()) {
...
async function saveProgress(progress: TrainerProgress) {
 ...
 if (error) {
 console.error('Failed to save KQKP7 progress:', error)
 }
}
...
function getChunkSortNumber(chunkFile: string, fallbackNumber: number) {
 const chunkMatch = chunkFile.match(/chunk[_-]?(\d+)/i)
...
function resolveChunkFiles(themeConfig: ThemeConfig) {
 const files = themeConfig.chunkFiles ?? themeConfig.chunks ?? []

 // Some KQKP7 progression files were generated with chunk names out of order.
 // Always normalize to natural numeric chunk order so Prev/Next cannot jump
 // evaluation_chunk_4 -> evaluation_chunk_3 or similar.
 return [...files].sort((a, b) => {
 const aNumber = getChunkSortNumber(a, 0)
 const bNumber = getChunkSortNumber(b, 0)
...
async function findFirstIncompleteChunk(
 ...
 `/data/endgames/kqkp7/chunks/${chunkFile}`
 )
...
export default function KQKP7Trainer() {
 ...
 if (loadingChunk || inputLocked || allComplete || phase === 'classify') return
 ...
 function shouldAskWinDraw() {
 return currentTrainingMode() === 'evaluate'
 }
 ...
 setMessage(position.explanation || 'Convert the win. Capture the pawn or force mate.')
 ...
 if (!progress || !progression || !currentThemeId || !currentThemeProgress) return

 const prevChunkIndex = currentThemeProgress.currentChunkIndex - 1

 if (prevChunkIndex >= 0) {
 goToThemeChunk(progress.currentThemeIndex, prevChunkIndex)
 return
 }

 const prevThemeIndex = progress.currentThemeIndex - 1
 if (prevThemeIndex < 0) return

 const prevThemeId = progression.order[prevThemeIndex]
 if (!prevThemeId) return

 const prevThemeConfig = buildThemeConfig(progression, prevThemeId)
 const prevChunkFiles = resolveChunkFiles(prevThemeConfig)
 if (prevChunkFiles.length === 0) return

 goToThemeChunk(prevThemeIndex, prevChunkFiles.length - 1)
 }

 function goToNextChunk() {
 if (!progress || !progression || !currentThemeId || !currentThemeProgress) return

 const nextChunkIndex = currentThemeProgress.currentChunkIndex + 1

 if (nextChunkIndex < currentChunkFiles.length) {
 goToThemeChunk(progress.currentThemeIndex, nextChunkIndex)
 return
 }
 ...
 goToThemeChunk(nextThemeIndex, 0)
 }
 ...
 '/data/endgames/kqkp7/kqkp7_progression.json'
 )
 ...
 const rawData = await fetchJson<any>(
 `/data/endgames/kqkp7/chunks/${currentChunkFile}`
 )
 ...
 setStatus(shouldAskWinDraw() ? 'Position restarted. Win or Draw?' : 'Position restarted. Convert the win.')
 }
 ...
 if (error) {
 console.error('Failed to restart KQKP7 chunk:', error)
 }
 }
 ...
 if (error) {
 console.error('Failed to reset KQKP7 progress:', error)
 }
 })()
 ...
 phase: "endgame",
 source: "trainer",
 trainerGoal: "Queen vs Pawn conversion",
 })
 } catch {
 ...
 setMessage('Try again. Convert the win.')
 setLastMove({})
 clearHighlights()
 setInputLocked(false)
 setMoveTimesMs([])
 ...
 setProgress(updatedProgress)
 setInputLocked(true)
 setStatus(currentPosition.result === 'draw' ? 'Correct -  Draw.' : 'Correct -  Win.')
 setMessage(
 ...
 if (!currentPosition || !progress || loadingChunk || loadError || allComplete) return
 ...
 function getPieceSquare(gameToCheck: Chess, type: 'k' | 'q' | 'p', color: 'w' | 'b') {
 ...
 if (!whiteQueenSquare) return false

 const blockers = new Set<string>()
 if (whiteKingSquare) blockers.add(whiteKingSquare)
 if (blackKingSquare) blockers.add(blackKingSquare)

 return queenAttacksSquare(whiteQueenSquare, blackPawnSquare, blockers)
 }
 ...
 function whiteQueenAttacksBlackPawn(gameToCheck: Chess) {
 const blackPawnSquare = getPieceSquare(gameToCheck, 'p', 'b')
 const whiteQueenSquare = getPieceSquare(gameToCheck, 'q', 'w')
 const whiteKingSquare = getPieceSquare(gameToCheck, 'k', 'w')
 const blackKingSquare = getPieceSquare(gameToCheck, 'k', 'b')

 if (!blackPawnSquare || !whiteQueenSquare) return false
 ...
 function chooseBestBlackDefense(gameToDefend: Chess, engineBestMove?: string | null) {
 ...
 const uci = moveToUci({ from: move.from, to: move.to, promotion: move.promotion })
 ...
 if (whiteQueenAttacksBlackPawn(next) && !blackKingProtectsPawn(next)) score -= 1500
 if (whiteCanCaptureBlackPawn(next) && !blackKingProtectsPawn(next)) score -= 1200
 }
 ...
 const defensiveReply = chooseBestBlackDefense(afterUserGame, replyInfo?.bestMove ?? null)
 ...
 if (loadingChunk || !currentPosition) return
 ...
 // Fast KQKP7 rules first -  no engine needed.
 ...
 const strictChunk = false
 const hasPreparedRoute = false
 ...
 You finished the full KQ vs K + pawn on 7th progression.
 </div>
 ...
 const sideToMoveText = phase === 'classify' ? 'Evaluate' : game.turn() === 'b' ? 'Black' : 'White'
 ...
 <div style={{ fontSize: 13, color: '#d8d8d8', lineHeight: 1.45, marginBottom: 10 }}>
 Is this position winning for the queen side, or drawn?
 </div>
 <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8 }}>
 <PrimaryButton onClick={() => handleClassificationAnswer('win')}>
 Win
 </PrimaryButton>
 <SecondaryButton onClick={() => handleClassificationAnswer('draw')}>
 Draw
 </SecondaryButton>
 </div>
 </PanelCard>
 ...
 message={status || getInstructionText(currentPosition, currentThemeConfig)}
 />
 ...
 <div>Result: {currentPosition.result === 'win' ? 'Win' : 'Draw'}</div>
