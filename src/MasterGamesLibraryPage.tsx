...
export default function MasterGamesLibraryPage() {
 ...
 const morphyAlias =
 normalizedPlayer.includes('morphy') ||
 normalizedPlayer.includes('murphy') ||
 normalizedPlayer.includes('paul morphy') ||
 normalizedPlayer.includes('paul murphy') ||
 normalizedPlayer.includes('morphy, paul')

 const operaSearch = normalizedQuery.includes('opera')

 return {
 ...debouncedFilters,
 query: operaSearch ? 'duke' : sameQueryAndPlayer ? '' : debouncedFilters.query,
 player: operaSearch ? '' : morphyAlias ? 'Morphy' : debouncedFilters.player,
 yearFrom: operaSearch ? '1858' : debouncedFilters.yearFrom,
 yearTo: operaSearch ? '1858' : debouncedFilters.yearTo,
 }
 }, [debouncedFilters])
