...
const OUT_DIR = path.join(ROOT, "public", "data", "pattern-mates", "smothered", "mate-in-4");

const SOURCE_CANDIDATES = [
 path.join(ROOT, "theme_exports", "mates", "mateIn4.csv"),
 path.join(ROOT, "lichess_db_puzzle.csv"),
];

const MAX_PUZZLES = 1500;
const CHUNK_SIZE = 30;
const MATE_DISTANCE = 4;
const SOLUTION_MOVE_COUNT = 7;
...
async function collectPuzzles(csvPath) {
 ...
 if (!themeSet.has("mateIn4")) continue;

 const movesText = getCell(row, ["Moves", "moves"]);
 const moveTokens = movesText.trim().split(/\s+/).filter(Boolean);

 // Lichess format: preMove + 7 solution moves
 if (moveTokens.length < 8) continue;

 const preMove = moveTokens[0];
 const solutionLine = moveTokens.slice(1, 8);
 ...
 puzzles.push({
 id: puzzleId ? `smothered-mate-in-4-${puzzleId}` : `smothered-mate-in-4-${puzzles.length + 1}`,
 puzzleId,
 fen: trainerFen,
...
async function main() {
 ...
 if (puzzles.length === 0) {
 throw new Error("No smothered mate in 4 puzzles found.");
 }
