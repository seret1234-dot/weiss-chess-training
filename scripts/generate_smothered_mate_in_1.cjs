...
const OUT_DIR = path.join(ROOT, "public", "data", "pattern-mates", "smothered", "mate-in-1");

const SOURCE_CANDIDATES = [
 path.join(ROOT, "theme_exports", "mates", "mateIn1.csv"),
 path.join(ROOT, "lichess_db_puzzle.csv"),
];

const MAX_PUZZLES = 1500;
const CHUNK_SIZE = 30;
const MATE_DISTANCE = 1;
const SOLUTION_MOVE_COUNT = 1;
...
async function collectPuzzles(csvPath) {
 ...
 const solutionResult = playUci(verifyChess, solutionLine[0]);
 if (!solutionResult) continue;
 ...
 puzzles.push({
 id: puzzleId ? `smothered-mate-in-1-${puzzleId}` : `smothered-mate-in-1-${puzzles.length + 1}`,
 puzzleId,
 fen: trainerFen,
...
async function main() {
 ...
 if (puzzles.length === 0) {
 throw new Error("No smothered mate in 1 puzzles found.");
 }
 ...
 file,
 count: chunk.length,
 minRating: Math.min(...chunk.map((puzzle) => puzzle.rating)),
 maxRating: Math.max(...chunk.map((puzzle) => puzzle.rating)),
 });
 }
 ...
 files: chunkFiles.map((chunk) => chunk.file),
 note: "Smothered Mate in 1",
 });
