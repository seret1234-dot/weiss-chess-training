...
const OUT_DIR = path.join(ROOT, "public", "data", "pattern-mates", "hook", "mate-in-5");

const SOURCE_CANDIDATES = [
 path.join(ROOT, "theme_exports", "mates", "mateIn5.csv"),
 path.join(ROOT, "lichess_db_puzzle.csv"),
];

const MAX_PUZZLES = 1500;
const CHUNK_SIZE = 30;
const MATE_DISTANCE = 5;
const SOLUTION_MOVE_COUNT = 9;
...
async function collectPuzzles(csvPath) {
 ...
 if (!themeSet.has("hookMate")) continue;
 if (!themeSet.has("mateIn5")) continue;

 const movesText = getCell(row, ["Moves", "moves"]);
 const moveTokens = movesText.trim().split(/\s+/).filter(Boolean);

 if (moveTokens.length < 10) continue;

 const preMove = moveTokens[0];
 const solutionLine = moveTokens.slice(1, 1 + SOLUTION_MOVE_COUNT);
 ...
 puzzles.push({
 id: puzzleId ? `hook-mate-in-5-${puzzleId}` : `hook-mate-in-5-${puzzles.length + 1}`,
 puzzleId,
 fen: trainerFen,
...
async function main() {
 ...
 if (puzzles.length === 0) {
 throw new Error("No hook mate in 5 puzzles found.");
 }
