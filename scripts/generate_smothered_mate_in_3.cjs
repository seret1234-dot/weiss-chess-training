...
const OUT_DIR = path.join(ROOT, "public", "data", "pattern-mates", "smothered", "mate-in-3");

const SOURCE_CANDIDATES = [
 path.join(ROOT, "theme_exports", "mates", "mateIn3.csv"),
 path.join(ROOT, "lichess_db_puzzle.csv"),
];

const MAX_PUZZLES = 1500;
const CHUNK_SIZE = 30;
const MATE_DISTANCE = 3;
const SOLUTION_MOVE_COUNT = 5;
...
async function collectPuzzles(csvPath) {
 ...
 if (!themeSet.has("mateIn3")) continue;

 const movesText = getCell(row, ["Moves", "moves"]);
 const moveTokens = movesText.trim().split(/\s+/).filter(Boolean);

 // Lichess format: preMove + 5 solution moves.
 // User moves are indexes 0, 2, 4 inside solutionLine.
 if (moveTokens.length < 6) continue;

 const preMove = moveTokens[0];
 const solutionLine = moveTokens.slice(1, 6);
 ...
 puzzles.push({
 id: puzzleId ? `smothered-mate-in-3-${puzzleId}` : `smothered-mate-in-3-${puzzles.length + 1}`,
 puzzleId,
 fen: trainerFen,
...
async function main() {
 ...
 if (puzzles.length === 0) {
 throw new Error("No smothered mate in 3 puzzles found.");
 }
