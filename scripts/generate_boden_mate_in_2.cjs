...
if (!csvPath) throw new Error("Could not find a Lichess puzzle CSV")
...
rl.on("line", (line) => {
 ...
 if (!themes.includes("bodenMate")) return
 ...
 // preMove + 3 solution moves for mate in 2
...
rl.on("close", () => {
 ...
 // Dedupe by exact FEN to avoid repeated positions
 const seen = new Set()
 const uniquePuzzles = []
 ...
 puzzles.length = 0
 puzzles.push(...uniquePuzzles.slice(0, maxPuzzles))
 ...
 label: `Boden Pattern ${globalIndex}`,
 theme: "Boden Mate",
 chunkNumber,
 ...
 source: "lichess",
 localId: `boden_mate_2_chunk_${String(chunkNumber).padStart(3, "0")}_puzzle_${indexInChunk + 1}`,
 ...
 distribution: "easy_to_hard_by_rating_unique_fen",
 files,
 }
 ...
 console.log("Unique FEN:", new Set(puzzles.map((p) => p.fen)).size)
