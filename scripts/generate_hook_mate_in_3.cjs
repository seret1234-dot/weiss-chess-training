...
const OUT_DIR = path.join(ROOT, "public", "data", "pattern-mates", "hook", "mate-in-3");
...
async function collectPuzzles(csvPath) {
 ...
 puzzles.push({
 id: puzzleId ? `hook-mate-in-3-${puzzleId}` : `hook-mate-in-3-${puzzles.length + 1}`,
 puzzleId,
 fen: trainerFen,
...
async function main() {
 ...
 if (puzzles.length === 0) {
 throw new Error("No hook mate in 3 puzzles found.");
 }
