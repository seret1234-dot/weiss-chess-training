...
try {
  ...
  assert.equal(solutionDisclosure.terminal, true, "Show Solution remains a terminal disclosure")
} finally {
  await vite.close()
}
...
assert.match(trainer, /scheduleCorrectAutoAdvance\(goToNextPuzzle\)/, "correct Tier A answers use the shared one-second auto-advance")
assert.doesNotMatch(trainer, /scheduleSemanticAutoAdvance/, "wrong answers and solution reveals do not start a terminal semantic timer")
assert.match(trainer, /cancelCorrectAutoAdvance\(\)/, "manual Next Puzzle, puzzle changes, restart, and unmount cancel the shared timer")
