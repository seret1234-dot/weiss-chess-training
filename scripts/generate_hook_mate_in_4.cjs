...
const OUT_DIR = path.join(ROOT, "public", "data", "pattern-mates", "hook", "mate-in-4");
...
async function collectPuzzles(csvPath) {
 ...
 puzzles.push({
 id: puzzleId ? `hook-mate-in-4-${puzzleId}` : `hook-mate-in-4-${puzzles.length + 1}`,
 puzzleId,
 fen: trainerFen,
...
async function main() {
 ...
 if (puzzles.length === 0) {
 throw new Error("No hook mate in 4 puzzles found.");
 }
