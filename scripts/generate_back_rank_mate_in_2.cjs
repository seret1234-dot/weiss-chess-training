...
rl.on("line", (line) => {
 ...
 // preMove + user move + reply + user mate
 if (moveTokens.length < 4) return
...
rl.on("close", () => {
 ...
 solutionLine: puzzle.moves.slice(1, 4),
 userMoveIndexes: [0, 2],
 ...
 source: "lichess",
 localId: `back_rank_mate_2_chunk_${String(chunkNumber).padStart(3, "0")}_puzzle_${indexInChunk + 1}`,
