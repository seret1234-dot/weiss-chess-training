...
rl.on("line", (line) => {
 ...
 if (!themes.includes("backRankMate")) return
 if (!themes.includes("mateIn3")) return
 ...
 // Lichess format here:
 // preMove + solution line.
 // Mate in 3 needs 1 preMove + 5 solution moves.
...
rl.on("close", () => {
 ...
 label: `Back Rank Pattern ${globalIndex}`,
 ...
 source: "lichess",
 localId: `back_rank_mate_3_chunk_${String(chunkNumber).padStart(3, "0")}_puzzle_${indexInChunk + 1}`,
